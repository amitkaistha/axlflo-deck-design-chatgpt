 ...
 * - v5.5 principles: card density tiers, semantic icon mapping, visual rhythm, number-led moments, strip/icon balance.
...
const pptxPath = path.join(outDir, "shift_to_scale_veda_proof_v55_0.pptx");
const manifestPath = path.join(outDir, "shift_to_scale_veda_proof_v55_0_manifest.json");
...
function drawIcon(sl, type, x, y, s, color, bg=C.cardNavy) {
  // Editable PowerPoint shape icons. Contextual, not generic glyphs/images.
  const lw = 1.15;
  const noFill = { color:bg, transparency:100 };
  // Safe line: pptxgenjs can emit corrupt OOXML when line extents are negative.
  // Always emit positive extents and use flipH/flipV to preserve direction.
  const addLine = (x1,y1,x2,y2,w=lw) => {
    ...
    sl.addShape(pptx.ShapeType.line, opt);
  };
  const addNode = (cx,cy,r=0.035) => sl.addShape(pptx.ShapeType.ellipse, { x:cx-r, y:cy-r, w:r*2, h:r*2, fill:{ color }, line:{ color } });

  if (type === "workflow" || type === "system") {
    // Three connected nodes: workflow/system architecture.
    addNode(x+s*0.17, y+s*0.22); addNode(x+s*0.58, y+s*0.22); addNode(x+s*0.38, y+s*0.68);
    addLine(x+s*0.20, y+s*0.24, x+s*0.54, y+s*0.24);
    addLine(x+s*0.56, y+s*0.26, x+s*0.41, y+s*0.63);
    addLine(x+s*0.35, y+s*0.64, x+s*0.18, y+s*0.27);
  } else if (type === "faultLine" || type === "brokenSystem") {
    // Broken process: split panel with fault line.
    sl.addShape(pptx.ShapeType.rect, { x, y:y+s*0.08, w:s*0.76, h:s*0.58, fill:noFill, line:{ color, width:lw } });
    addLine(x+s*0.36, y+s*0.09, x+s*0.25, y+s*0.34, lw);
    addLine(x+s*0.25, y+s*0.34, x+s*0.45, y+s*0.34, lw);
    addLine(x+s*0.45, y+s*0.34, x+s*0.34, y+s*0.65, lw);
  } else if (type === "divergence" || type === "gap") {
    // Diverging path: gap widening.
    addLine(x+s*0.10, y+s*0.68, x+s*0.38, y+s*0.40);
    addLine(x+s*0.38, y+s*0.40, x+s*0.72, y+s*0.18);
    addLine(x+s*0.38, y+s*0.40, x+s*0.76, y+s*0.70);
    addLine(x+s*0.65, y+s*0.18, x+s*0.72, y+s*0.18);
    addLine(x+s*0.72, y+s*0.18, x+s*0.72, y+s*0.25);
    addLine(x+s*0.69, y+s*0.70, x+s*0.76, y+s*0.70);
    addLine(x+s*0.76, y+s*0.70, x+s*0.76, y+s*0.63);
  } else if (type === "prompt") {
    // Prompt dependency: chat bubble + cursor.
    sl.addShape(pptx.ShapeType.roundRect, { x, y:y+s*0.15, w:s*0.68, h:s*0.42, rectRadius:0.03, fill:noFill, line:{ color, width:lw } });
    addLine(x+s*0.16, y+s*0.34, x+s*0.42, y+s*0.34);
    addLine(x+s*0.50, y+s*0.46, x+s*0.75, y+s*0.70);
  } else if (type === "context") {
    // Context stack: layered files.
    sl.addShape(pptx.ShapeType.rect, { x:x+s*0.06, y:y+s*0.14, w:s*0.54, h:s*0.42, fill:noFill, line:{ color, width:lw } });
    sl.addShape(pptx.ShapeType.rect, { x:x+s*0.18, y:y+s*0.26, w:s*0.54, h:s*0.42, fill:noFill, line:{ color, width:lw } });
    addLine(x+s*0.26, y+s*0.40, x+s*0.58, y+s*0.40);
  } else if (type === "drift") {
    // Drift: offset target/rings.
    sl.addShape(pptx.ShapeType.ellipse, { x:x+s*0.10, y:y+s*0.16, w:s*0.58, h:s*0.58, fill:noFill, line:{ color, width:lw } });
    sl.addShape(pptx.ShapeType.ellipse, { x:x+s*0.27, y:y+s*0.33, w:s*0.24, h:s*0.24, fill:noFill, line:{ color, width:lw } });
    addLine(x+s*0.55, y+s*0.22, x+s*0.78, y+s*0.10);
  } else if (type === "doc" || type === "execution") {
    // Execution card/document.
    sl.addShape(pptx.ShapeType.rect, { x:x+s*0.10, y:y+s*0.08, w:s*0.58, h:s*0.70, fill:noFill, line:{ color, width:lw } });
    [0.26,0.42,0.58].forEach(fr=>addLine(x+s*0.20, y+s*fr, x+s*0.58, y+s*fr));
  } else if (type === "network" || type === "champions") {
    // Champions network.
    const pts = [[0.14,0.26],[0.72,0.30],[0.40,0.72]];
    pts.forEach(p=>addNode(x+s*p[0], y+s*p[1]));
    addLine(x+s*pts[0][0], y+s*pts[0][1], x+s*pts[1][0], y+s*pts[1][1]);
    addLine(x+s*pts[1][0], y+s*pts[1][1], x+s*pts[2][0], y+s*pts[2][1]);
    addLine(x+s*pts[2][0], y+s*pts[2][1], x+s*pts[0][0], y+s*pts[0][1]);
  } else if (type === "governance" || type === "shield") {
    // Governance/shield.
    sl.addShape(pptx.ShapeType.homePlate, { x:x+s*0.10, y:y+s*0.06, w:s*0.62, h:s*0.72, fill:noFill, line:{ color, width:lw }, rotate:180 });
    addLine(x+s*0.27, y+s*0.40, x+s*0.37, y+s*0.52);
    addLine(x+s*0.37, y+s*0.52, x+s*0.58, y+s*0.27);
  } else if (type === "dataLock") {
    // Storage + permissions: database stack + lock shackle.
    sl.addShape(pptx.ShapeType.can, { x:x+s*0.08, y:y+s*0.12, w:s*0.45, h:s*0.50, fill:noFill, line:{ color, width:lw } });
    sl.addShape(pptx.ShapeType.roundRect, { x:x+s*0.50, y:y+s*0.28, w:s*0.28, h:s*0.36, rectRadius:0.02, fill:noFill, line:{ color, width:lw } });
    addLine(x+s*0.56, y+s*0.42, x+s*0.72, y+s*0.42);
  } else if (type === "fileLayers") {
    // Flat file as operating system.
    sl.addShape(pptx.ShapeType.rect, { x:x+s*0.08, y:y+s*0.12, w:s*0.56, h:s*0.64, fill:noFill, line:{ color, width:lw } });
    [0.30,0.47,0.64].forEach(fr=>addLine(x+s*0.18, y+s*fr, x+s*0.55, y+s*fr));
  } else if (type === "automation") {
    // AI engine: gear + small node.
    sl.addShape(pptx.ShapeType.gear6, { x:x+s*0.08, y:y+s*0.10, w:s*0.58, h:s*0.58, fill:noFill, line:{ color, width:lw } });
    addNode(x+s*0.73, y+s*0.24, 0.025);
  } else if (type === "memory") {
    // Institutional memory/archive.
    sl.addShape(pptx.ShapeType.rect, { x:x+s*0.10, y:y+s*0.18, w:s*0.62, h:s*0.48, fill:noFill, line:{ color, width:lw } });
    addLine(x+s*0.18, y+s*0.33, x+s*0.62, y+s*0.33);
    addLine(x+s*0.18, y+s*0.48, x+s*0.50, y+s*0.48);
  } else if (type === "onboarding") {
    // Onboarding: user + pathway.
    sl.addShape(pptx.ShapeType.ellipse, { x:x+s*0.12, y:y+s*0.16, w:s*0.20, h:s*0.20, fill:noFill, line:{ color, width:lw } });
    sl.addShape(pptx.ShapeType.rect, { x:x+s*0.08, y:y+s*0.45, w:s*0.28, h:s*0.18, fill:noFill, line:{ color, width:lw } });
    addLine(x+s*0.44, y+s*0.42, x+s*0.76, y+s*0.42);
    addLine(x+s*0.68, y+s*0.34, x+s*0.76, y+s*0.42);
    addLine(x+s*0.68, y+s*0.50, x+s*0.76, y+s*0.42);
  } else {
    sl.addShape(pptx.ShapeType.ellipse, { x, y, w:s*0.68, h:s*0.68, fill:noFill, line:{ color, width:lw } });
  }
}
...
function addCard(sl, x, y, w, h, stripe, heading, body, opts={}) {
  ...
  const fsH = opts.headingSize || 12.2;
  const fsB = opts.bodySize || 9.8;
  const iconType = opts.iconType;
  const iconSize = opts.iconShapeSize || 0.42;
  const padX = opts.padX || 0.18;
  const padTop = opts.padTop || 0.20;

  sl.addShape(pptx.ShapeType.rect, { x, y, w, h, fill:{ color:fill }, line:{ color:line, width:0.5 }, shadow:E.shadow() });
  if (stripe && !opts.noStrip) sl.addShape(pptx.ShapeType.rect, { x, y, w:STRIP, h, fill:{ color:stripe } });

  let titleY = y + padTop;
  if (iconType) {
    drawIcon(sl, iconType, x+padX, y+padTop, iconSize, opts.iconColor || C.white, fill);
    titleY = y + padTop + iconSize + 0.18;
  }

  if (heading) {
    sl.addText(heading, { x:x+padX, y:titleY, w:w-padX*2, h:opts.headingH || 0.34, fontFace:"Segoe UI", fontSize:fsH, bold:true, color:headingColor, margin:0, fit:"shrink" });
  }
  if (body) {
    const bodyY = titleY + (opts.headingH || 0.34) + 0.10;
    sl.addText(body, { x:x+padX, y:bodyY, w:w-padX*2, h:Math.max(0.2, h-(bodyY-y)-0.16), fontFace:"Segoe UI", fontSize:fsB, color:bodyColor, margin:0, fit:"shrink", breakLine:false, valign:"top" });
  }
}
...
function addSlide3(gb) {
  ...
  const cards = [
    ["prompt", "The Prompt Dependency", "Output quality is determined by who's asking. The power user gets great results. Everyone else gets noise. The gap widens every week."],
    ["context", "Context Lives in the Conversation", "Rules exist in someone's head. Every session starts from scratch. There's no institutional memory — no way to make expert learning available to the next person."],
    ["drift", "Drift Is Invisible Until It's Expensive", "Rules accumulate informally. Outputs become inconsistent. Nobody knows what changed or when. The system looked fine until it didn't."]
  ];
  cards.forEach((c,i)=>addCard(sl, 0.28+i*3.18, 1.35, 2.95, 2.55, SEQ[3][i], c[1], c[2], { iconType:c[0], iconShapeSize:0.42, bodySize:10.25, headingSize:11.6 }));
...
function addSlide5(gb) {
  ...
  const cards = [
    ["system", "Strong systems get stronger.", "A well-designed workflow through AI produces consistent, high-quality output at speed. It compounds — every user who follows the system makes it better."],
    ["brokenSystem", "Broken systems break faster.", "A disorganised process run through AI produces disorganised output — at higher volume and higher speed. AI doesn't fix broken. It scales it."],
    ["gap", "The gap widens by default.", "Without a system, your best people get better. Everyone else stays flat. The adoption gap is structural — it does not close on its own."]
  ];
  cards.forEach((c,i)=>addCard(sl, 0.28+i*3.18, 1.63, 2.95, 2.65, SEQ[3][i], c[1], c[2], { iconType:c[0], iconShapeSize:0.42, bodySize:10.0, headingSize:11.5 }));
...
function addSlide6(gb) {
  ...
  sl.addText("Most organisations are searching for the right tool. The right licence. The right vendor. The right dashboard.\n\nThat's the wrong search.\n\nThe companies getting real results from AI are not running better software. They are running better systems. Structured context — logic, rules, governance — stored in a form that AI can read, execute against, and maintain consistently across every user, every session, every day.", { x:0.35, y:1.90, w:5.35, h:2.12, fontFace:"Segoe UI", fontSize:11.8, color:C.white, margin:0, fit:"shrink", breakLine:false });

  // Compact concept rows: icon and text share a precise baseline. This is the exception used when cards are too shallow for the full icon-above-title pattern.
  const blocks = [
    { icon:"dataLock", title:"Storage + permissions + logic", body:"the new software", color:C.indigoBlue },
    { icon:"fileLayers", title:"The flat file", body:"the operating system", color:C.purple },
    { icon:"automation", title:"The AI", body:"the engine", color:C.fieryCoral }
  ];
  blocks.forEach((b,i)=>{
    const x=6.10, y=1.80+i*0.88, w=3.45, h=0.70;
    sl.addShape(pptx.ShapeType.rect, { x, y, w, h, fill:{ color:C.cardNavy }, line:{ color:C.cardBorder, width:0.5 }, shadow:E.shadow() });
    sl.addShape(pptx.ShapeType.rect, { x, y, w:STRIP, h, fill:{ color:b.color } });
    drawIcon(sl, b.icon, x+0.20, y+0.17, 0.30, C.white, C.cardNavy);
    sl.addText(b.title, { x:x+0.72, y:y+0.17, w:w-0.95, h:0.20, fontFace:"Segoe UI", fontSize:10.8, bold:true, color:C.white, margin:0, fit:"shrink" });
    sl.addText(b.body, { x:x+0.72, y:y+0.43, w:w-0.95, h:0.15, fontFace:"Segoe UI", fontSize:9.2, color:C.coolGrey, margin:0, fit:"shrink" });
  });
...
function addSlide10(gb) {
  ...
  const cards = [
    ["workflow", "Consistent output across every operator.", "The best practice is in the file, not the person. Everyone executes from the same playbook."],
    ["memory", "Institutional memory that compounds.", "Every refinement, corrected edge case and lesson learned is documented, versioned and available from day one."],
    ["onboarding", "Onboarding time cut dramatically.", "New team members don't start from scratch. They start from the system. Ramp time measured in days, not quarters."]
  ];
  cards.forEach((c,i)=>addCard(sl, 0.28+i*3.18, 2.05, 2.95, 1.62, SEQ[3][i], c[1], c[2], {
    iconType:c[0], iconShapeSize:0.32, headingSize:10.5, bodySize:8.8, padTop:0.16, padX:0.18, headingH:0.30
  }));
...
function addSlide12(gb) {
  const sl = pptx.addSlide(); darkBase(sl, gb); addDarkTitle(sl, "HOW IT SCALES", "Built to Scale. Not Because It's Big. Because It's Designed Right.", 24.5);
  sl.addText("Most AI programmes are designed for pilots. This is designed for production. The difference is not ambition. It is architecture.", { x:0.30, y:1.14, w:9.0, h:0.28, fontFace:"Segoe UI", fontSize:11.1, italic:true, color:C.coolGrey, margin:0, fit:"shrink" });

  const cards = [
    { icon:"doc", title:"Systematised fluency.", body:"Role-specific execution cards take what one person figured out and make it available to five hundred.", range:"1–6", metric:"Use case definition + governance design", note:"10–20 pilot users", color:C.indigoBlue },
    { icon:"network", title:"A champions network that compounds.", body:"Champions are the feedback loop — collecting what breaks, what works, and what the system doesn't yet cover.", range:"7–16", metric:"Champions network activated", note:"100–300 users", color:C.purple },
    { icon:"governance", title:"Governance that grows with the organisation.", body:"The rulebook, QA gate and changelog get more valuable with every version.", range:"17–52", metric:"Full deployment", note:"Enterprise-wide with documented ROI", color:C.fieryCoral }
  ];
  ...
  cards.forEach((c,i)=>{
    ...
    const h = 2.82;
    sl.addShape(pptx.ShapeType.rect, { x, y, w, h, fill:{ color:C.cardNavy }, line:{ color:C.cardBorder, width:0.5 }, shadow:E.shadow() });
    sl.addShape(pptx.ShapeType.rect, { x, y, w:STRIP, h, fill:{ color:c.color } });
    drawIcon(sl, c.icon, x+0.22, y+0.25, 0.42, C.white, C.cardNavy);
    sl.addText(c.title, { x:x+0.22, y:y+0.82, w:w-0.44, h:0.34, fontFace:"Segoe UI", fontSize:10.8, bold:true, color:C.white, margin:0, fit:"shrink" });
    sl.addText(c.body, { x:x+0.22, y:y+1.22, w:w-0.44, h:0.62, fontFace:"Segoe UI", fontSize:8.9, color:C.coolGrey, margin:0, fit:"shrink", breakLine:false, valign:"top" });
    sl.addShape(pptx.ShapeType.rect, { x:x+0.22, y:y+2.05, w:w-0.44, h:0.01, fill:{ color:c.color } });
    sl.addText(c.range, { x:x+0.22, y:y+2.16, w:0.92, h:0.34, fontFace:"Segoe UI", fontSize:23, bold:true, color:c.color, margin:0, fit:"shrink", align:"left" });
    sl.addText(c.metric + "\n" + c.note, { x:x+1.15, y:y+2.17, w:w-1.42, h:0.34, fontFace:"Segoe UI", fontSize:7.5, bold:true, color:C.white, margin:0, fit:"shrink", breakLine:false });
  });
...
function addSlide14(gb) {
  ...
  sl.addText("Built on AxlFlo Deck System v5.5", { x:0.45, y:5.03, w:4.0, h:0.16, fontFace:"Segoe UI", fontSize:8, italic:true, color:C.mustardGold, margin:0, fit:"shrink" });
