...
const pptxPath = path.join(outDir, "shift_to_scale_veda_proof_v54_4.pptx");
const manifestPath = path.join(outDir, "shift_to_scale_veda_proof_v54_4_manifest.json");
...
const BUILD_VERSION = "Deck build v5.4.4 · pptxgenjs 3.12.0 · May 2026";
...
function drawIcon(sl, type, x, y, s, color, bg=C.cardNavy) {
  ...
  if (type === "system") {
    sl.addShape(pptx.ShapeType.gear6, { x, y, w:s, h:s, fill:noFill, line:{ color, width:lw } });
  } else if (type === "brokenSystem") {
    sl.addShape(pptx.ShapeType.rect, { x, y:y+0.03, w:s*0.74, h:s*0.52, fill:noFill, line:{ color, width:lw } });
    sl.addShape(pptx.ShapeType.line, { x:x+s*0.14, y:y+s*0.09, w:s*0.36, h:s*0.36, line:{ color, width:lw } });
    sl.addShape(pptx.ShapeType.line, { x:x+s*0.50, y:y+s*0.09, w:-s*0.36, h:s*0.36, line:{ color, width:lw } });
  } else if (type === "gap") {
    const bw = s*0.16;
    [0.28,0.52,0.78].forEach((hFrac,i)=>{
      sl.addShape(pptx.ShapeType.rect, { x:x+i*s*0.25, y:y+s*(1-hFrac), w:bw, h:s*hFrac, fill:{ color }, line:{ color } });
    });
  } else if (type === "doc") {
    sl.addShape(pptx.ShapeType.rect, { x, y, w:s*0.72, h:s*0.78, fill:noFill, line:{ color, width:lw } });
    [0.22,0.40,0.58].forEach(fr=>sl.addShape(pptx.ShapeType.line, { x:x+s*0.14, y:y+s*fr, w:s*0.44, h:0, line:{ color, width:lw } }));
  } else if (type === "network") {
    const pts = [[0.10,0.18],[0.70,0.28],[0.38,0.72]];
    sl.addShape(pptx.ShapeType.line, { x:x+s*pts[0][0], y:y+s*pts[0][1], w:s*(pts[1][0]-pts[0][0]), h:s*(pts[1][1]-pts[0][1]), line:{ color, width:lw } });
    sl.addShape(pptx.ShapeType.line, { x:x+s*pts[1][0], y:y+s*pts[1][1], w:s*(pts[2][0]-pts[1][0]), h:s*(pts[2][1]-pts[1][1]), line:{ color, width:lw } });
    sl.addShape(pptx.ShapeType.line, { x:x+s*pts[2][0], y:y+s*pts[2][1], w:s*(pts[0][0]-pts[2][0]), h:s*(pts[0][1]-pts[2][1]), line:{ color, width:lw } });
    pts.forEach(p=>sl.addShape(pptx.ShapeType.ellipse, { x:x+s*p[0]-0.035, y:y+s*p[1]-0.035, w:0.07, h:0.07, fill:{ color }, line:{ color } }));
  } else if (type === "governance") {
    sl.addShape(pptx.ShapeType.hexagon, { x, y, w:s*0.76, h:s*0.76, fill:noFill, line:{ color, width:lw } });
    sl.addShape(pptx.ShapeType.line, { x:x+s*0.22, y:y+s*0.40, w:s*0.14, h:s*0.12, line:{ color, width:lw } });
    sl.addShape(pptx.ShapeType.line, { x:x+s*0.35, y:y+s*0.52, w:s*0.24, h:-s*0.28, line:{ color, width:lw } });
  } else {
...
function addSlide5(gb) {
  ...
  cards.forEach((c,i)=>addCard(sl, 0.28+i*3.18, 1.63, 2.95, 2.65, SEQ[3][i], c[1], c[2], { iconType:c[0], iconShapeSize:0.42, bodySize:10.5, headingSize:11.7 }));
