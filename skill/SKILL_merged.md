...
### 13.5 Mandatory Post-Build Workflow

Every deck build must complete this workflow before being called finished:

1. **Build from source.** Generate the PPTX from the current source layer: JSON, markdown, or custom build script.
2. **Run structural QA.** Execute the repo QA script and report pass, warning, and failure counts.
3. **Run content/narrative QA.** Check whether the deck has a clear argument, one idea per slide, CXO-level stakes, self-explanatory email-read copy, and no filler/framework repetition.
4. **Run visual/design QA.** Inspect slide anatomy for missing icons, duplicate cream quote boxes, footer misuse, color-rule violations, text density, rhythm, hierarchy, and layout polish.
5. **Report creative critique.** Always give a blunt post-build note: rating, what works, what is weak, what should improve next, and whether the deck is ready to send.

Structural QA is necessary but not sufficient. A deck can pass the script and still fail taste, narrative, or executive-readiness. The creative critique is part of the build artifact, not an optional afterthought.

### 13.6 Feedback Reinforcement Loop

Every reviewed deck iteration must capture feedback and use it to improve the system slowly and deliberately. Feedback does not become a permanent rule until Amit approves it.

Classification:

- **fix_now** — Concrete issue in the current deck. Patch, rebuild, and rerun QA.
- **promote_to_rule** — Reusable design/process improvement. Requires Amit approval before changing spec, renderer, QA, schema, or templates.
- **watchlist** — Useful observation, but not stable enough to become a rule.
- **reject** — Not adopted because it conflicts with brand, narrative, render safety, or deck purpose.

Mandatory feedback fields:

- Source: Amit, self-review, structural QA, content QA, or visual QA.
- Scope: current deck, deck family, or global system.
- Severity: blocker, important, polish, or idea.
- Decision: fix now, promote to rule, watchlist, or reject.
- Verification: how the next build proves it improved.

Approved feedback must be recorded in `docs/FEEDBACK_LOOP.md`. Every future deck build must check the approved feedback register before final delivery to prevent regression.
...
POST-BUILD: Build → structural QA → content/narrative QA → visual/design QA → blunt creative critique
FEEDBACK LOOP: capture → classify → Amit approval → promote rule/fix → rebuild → regression check
```
...
| **v5.7** | May 2026 | **Mandatory post-build workflow.** Every deck build now requires five steps: build from source, structural QA, content/narrative QA, visual/design QA, and blunt creative critique before the deck is considered finished. |
| **v5.8** | May 2026 | **Feedback reinforcement loop.** Every reviewed deck iteration now captures feedback, classifies it, requires Amit approval before rule promotion, records approved items in `docs/FEEDBACK_LOOP.md`, and checks prior approved feedback in later builds to prevent regression. |
